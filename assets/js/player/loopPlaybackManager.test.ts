...
describe('simulates playback', () => {
  ...
  test('for one loop', () => {
    ...
    for(let currTimestep = 0; currTimestep < mocks.shortLoop.length * 3; currTimestep++) {
      const dueTimesteps = lpm.getDueTimestepSlices(currTimestep);
      actualTimestepSlicesEachTimestep.push(dueTimesteps);
    }
  
    expect(actualTimestepSlicesEachTimestep).toStrictEqual(mocks.expectedTimestepSlicesShortLoop);
  });
  ...
  test('for multiple loops', () => {
    ...
    for(let currTimestep = 0; currTimestep < mocks.longLoop.length * 3; currTimestep++) {
      const dueTimesteps = lpm.getDueTimestepSlices(currTimestep);
      actualTimestepSliceLengths.push(dueTimesteps.length);
    }

    const expectedTimestepSliceLengths = [3,2,1,1,2,2,1,0,3,2,1,1,2,2,1,0,3,2,1,1,2,2,1,0];
  
    expect(actualTimestepSliceLengths).toStrictEqual(expectedTimestepSliceLengths);
  });
});
...
describe("getDueTimestepSlices", () => {
  test("when some timestep slices are due", () => {
    ...
    const expectedTimestepSlices: TimestepSlice[] = mocks.defaultLoop.timestep_slices.slice(1,4);
  ...
  test("when 1 timestep slice is due", () => {
    ...
    const expectedTimestepSlices: TimestepSlice[] = mocks.defaultLoop.timestep_slices.slice(0,1);
  ...
  test("when all timestep slices are due", () => {
    ...
    const expectedTimestepSlices: TimestepSlice[] = mocks.defaultLoop.timestep_slices.slice(1,100);
